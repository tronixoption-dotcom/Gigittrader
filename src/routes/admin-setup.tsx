...
export const Route = createFileRoute("/admin-setup")({
  ssr: false,
  head: () => ({ meta: [{ title: "Admin setup - DIGIT TRADER" }] }),
...
function AdminSetupPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success(result.promotedExisting ? "Existing user promoted to admin" : "Admin account created");
      setFullName("");
      setEmail("");
      setPassword("");
      setConfirm("");
    ...
    <div className="min-h-screen grid place-items-center bg-background px-4 py-8 text-foreground">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          <img src={LOGO_URL} alt="Digit Trader" className="mx-auto mb-2 h-14 w-14 object-contain" />
          <div className="text-xl font-extrabold tracking-wider">DIGIT<span className="text-primary"> TRADER</span></div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-2xl">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{unlocked ? "Register a new admin account." : "Enter the setup password to continue."}</p>
            </div>
          </div>
          ...
          {!unlocked ? (
            <form onSubmit={unlock} className="space-y-3">
              ...
              <button disabled={busy} className="w-full rounded-xl bg-primary py-2.5 text-sm font-bold text-primary-foreground glow-primary disabled:opacity-50">
            ...
            <form onSubmit={submit} className="space-y-3">
              <Field label="Full name">
                <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="setup-input" required minLength={2} autoComplete="name" />
              </Field>
              <Field label="Email">
                <input value={email} onChange={(e) => setEmail(e.target.value)} className="setup-input" required type="email" autoComplete="email" />
              </Field>
              ...
              <Field label="Confirm password">
                <input value={confirm} onChange={(e) => setConfirm(e.target.value)} className="setup-input" required type="password" autoComplete="new-password" />
              </Field>
              <button disabled={busy} className="w-full rounded-xl bg-primary py-2.5 text-sm font-bold text-primary-foreground glow-primary disabled:opacity-50">
          ...
          <Link to="/auth" className="mt-4 block text-center text-xs font-bold text-muted-foreground hover:text-foreground">
            Back to sign in
          </Link>
        </div>
      </div>
